...
class scTEL_API(object):
    def __init__(self, gene_trainsets, protein_trainsets, gene_test=None, gene_list=[], select_hvg=True,
                 ...
                 batch_size=32, val_split=0.1, use_gpu=True, h_size=512, h=4, drop_rate=0.15):
    ...
    def train(self, quantiles=[0.1, 0.25, 0.75, 0.9], n_epochs=10000, ES_max=12, decay_max=6,
        ...
        protein_loss = sse_quantile(self.device, quantiles)
        ...
        if load and isfile(path):
            ...
            train_params = (
                self.dataloaders['train'], self.dataloaders['val'], path, n_epochs, ES_max, decay_max, decay_step, lr)

            self.model.train_backprop(*train_params)
            save(self.model.state_dict(), path)

    def impute(self, requested_quantiles='all'):
        if requested_quantiles == 'all':
            requested_quantiles = self.quantiles
        else:
            assert type(requested_quantiles) == list

        return self.model.impute(self.dataloaders['impute'], requested_quantiles, self.proteins)

    def predict(self, requested_quantiles='all'):
        ...
        return self.model.predict(self.dataloaders['test'], requested_quantiles, self.proteins,
                                  self.test_cells)
