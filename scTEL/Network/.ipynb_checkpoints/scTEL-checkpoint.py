...
from torch.nn import Module, Linear, Sequential, Softplus, Parameter, Softmax, TransformerEncoderLayer, LSTMCell
from torch.optim import Adam
from torch.optim.lr_scheduler import StepLR

from .TE_Layer import Input_TE, FF_TE, LambdaLayer, Dual_Forward
...
class scTEL_Model(Module):
    def __init__(self, p_mod1, p_mod2, h_size, h, loss1, loss2, quantiles, categories, drop_rate = 0.2):
        # p_mod1: HVGs p_mod2:proteins
        super(scTEL_Model, self).__init__()

        h_size, drop_rate = h_size, drop_rate

        self.LSTMCell = LSTMCell(h_size, h_size)

        self.input_block = Input_TE(p_mod1, h_size, h, drop_rate, drop_rate)

        self.skip_1 = FF_TE(h_size, h, drop_rate)
        self.skip_2 = FF_TE(h_size, h, drop_rate)
        self.skip_3 = FF_TE(h_size, h, drop_rate)
        # self.skip_4 = FF_TE(h_size, h, drop_rate)
        # self.skip_5 = FF_TE(h_size, h, drop_rate)

        MSE_output = Linear(h_size, p_mod2)  # 512→224
    ...
    def forward_transfer(self, x):
        ...
        h, c= self.LSTMCell(x, (zeros_like(x),zeros_like(x)))
        ...
        return {'celltypes': self.celltype_out(c.detach()), 'modality 2': self.mod2_out(x), 'embedding': h}

    def forward_simple(self, x):
        x = self.input_block(x)  # (8,512)
        h, c = self.LSTMCell(x, (zeros_like(x), zeros_like(x)))  # (batch_size,512)
        ...
        # 'modality 2': protein expression(8,224) uncertainty(8,224,4)
        ...
        for epoch in range(n_epoch):
            with no_grad():
                ...
                if self.categories_arr is None:
                    ...
                    print(
                        f"Epoch {epoch} prediction loss = {running_loss / len(val_loader):.3f}, validation accuracy = {rtype_acc / len(val_loader):.3f}")
            ...
            for batch, inputs in enumerate(train_loader):
                ...
                mod1_loss = self.loss1(outputs['celltypes'], celltypes)  # 类别损失函数
                mod2_loss = self.loss2(outputs['modality 2'], mod2, protein_bools)
